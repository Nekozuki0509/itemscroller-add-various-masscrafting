...
public class InventoryUtils {
    ...
    public static void debugPrintSlotInfo(HandledScreen<? extends ScreenHandler> gui, Slot slot) {
        ...
        ItemScroller.logger.info(String.format(
                "slot: slotNumber: %d, getSlotIndex(): %d, getHasStack(): %s, "
    ...
    private static boolean isValidSlot(Slot slot, HandledScreen<? extends ScreenHandler> gui, boolean requireItems) {
        ...
        return container != null && container.slots != null && slot != null && container.slots.contains(slot)
    ...
    public static boolean tryMoveItems(HandledScreen<? extends ScreenHandler> gui,
        ...
        boolean villagerHandling = Configs.Toggles.SCROLL_VILLAGER.getBooleanValue() && gui instanceof MerchantScreen
        ...
        if (Configs.Generic.SLOT_POSITION_AWARE_SCROLL_DIRECTION.getBooleanValue()) {
            ...
            // so basically: (above && scrollingUp) || (above == false && scrollingUp ==
            // false)
        ...
        // Check that the slot is valid, (don't require items in case of the villager
        // output slot or a crafting slot)
        if (isValidSlot(slot, gui, villagerHandling == false && craftingHandling == false) == false) {
            return false;
        }

        if (craftingHandling) {
            return tryMoveItemsCrafting(recipes, slot, gui, moveToOtherInventory, keyActiveMoveStacks,
                    keyActiveMoveEverything);
        }
    ...
    public static boolean dragMoveItems(HandledScreen<? extends ScreenHandler> gui,
        ...
        if (isStackEmpty(gui.getScreenHandler().getCursorStack()) == false) {
            // Updating these here is part of the fix to preventing a drag after shift +
            // place
            lastPosX = mouseX;
            lastPosY = mouseY;
            stopDragging();
    ...
    private static boolean dragMoveFromSlotAtPosition(HandledScreen<? extends ScreenHandler> gui,
        ...
        // boolean cancel = flag && (amount == MoveAmount.LEAVE_ONE || amount ==
        // MoveAmount.MOVE_ONE);
    ...
    private static boolean dragMoveFromSlotAtPositionCreative(HandledScreen<? extends ScreenHandler> gui,
        ...
        if (flag && slotNumber != slotNumberLast && DRAGGED_SLOTS.contains(slotNumber) == false) {
            switch (action) {
                ...
                case MOVE_TO_OTHER_LEAVE_ONE:
                    // Too lazy to try to duplicate the proper code for the weird creative
                    // inventory...
                    if (isPlayerInv == false) {
                        leftClickSlot(guiCreative, slot, slotNumber);
                        rightClickSlot(guiCreative, slot, slotNumber);

                        // Delete the rest of the stack by placing it in the first creative "source
                        // slot"
    ...
    public static void dropMultipleStacks(HandledScreen<? extends ScreenHandler> gui, HashSet<Item> itemSet,
            Slot slotReference, boolean sameInventory) {
        if (slotReference != null) {
            ...
            for (Slot slot : container.slots) {
                ...
                if (areSlotsInSameInventory(slot, slotReference) == sameInventory
                        && itemSet.contains(slot.getStack().getItem())) {
                    // Drop the stack
                    dropStack(gui, slot.id);
                }
            }
    ...
    public static boolean shiftPlaceItems(Slot slot, HandledScreen<? extends ScreenHandler> gui) {
        ...
        // Ugly fix to prevent accidentally drag-moving the stack from the slot that it
        // was just placed into...
        DRAGGED_SLOTS.add(slot.id);
    ...
    /**
     * Store a reference to the slot when a slot is left or right clicked on. The
     * slot is then later used to determine which inventory an ItemStack was picked
     * up from, if the stack from the cursor is dropped while holding shift.
     */
    public static void storeSourceSlotCandidate(Slot slot, HandledScreen<?> gui) {
        ...
        if (slot != null) {
            ...
            // Store the candidate
            // NOTE: This method is called BEFORE the stack has been picked up to the
            // cursor!
            // Thus we can't check that there is an item already in the cursor, and that's
            // why this is just a "candidate"
    ...
    /**
     * Check if the (previous) mouse event resulted in picking up a new ItemStack to
     * the cursor
     */
    ...
    public static boolean villagerTradeEverythingPossibleWithAllFavoritedTrades() {
        ...
        if (screen instanceof MerchantScreen) {
            ...
            IntArrayList favorites = VillagerDataStorage.getInstance()
                    .getFavoritesForCurrentVillager(handler).favorites;
    ...
    private static boolean tryMoveAllButOneItemToOtherInventory(Slot slot,
        ...
        for (Slot slotTmp : gui.getScreenHandler().slots) {
            if (slotTmp.id != slot.id && areSlotsInSameInventory(slotTmp, slot, true)
                    && slotTmp.canInsert(stackInCursor) && slotTmp.canTakeItems(player)) {
        ...
        // No temporary slot found, try to move the stack manually
            ...
            IntArrayList slots = getSlotNumbersOfEmptySlots(gui.getScreenHandler(), slot, false, treatHotbarAsDifferent,
                    false);

            if (slots.isEmpty()) {
                slots = getSlotNumbersOfMatchingStacks(gui.getScreenHandler(), slot, false, slot.getStack(), true,
                        treatHotbarAsDifferent, false);
            }
            ...
            if (slots.isEmpty() == false) {
                ...
                // Try to place the stack in the cursor to any valid empty or matching slots in
    ...
    private static boolean tryMoveSingleItemToThisInventory(Slot slot,
        ...
        for (int slotNum = container.slots.size() - 1; slotNum >= 0; slotNum--) {
            ...
            if (areSlotsInSameInventory(slotTmp, slot) == false && isStackEmpty(stackTmp) == false
                    && slotTmp.canTakeItems(mc.player)
    ...
    private static void craftAsManyItemsAsPossible(RecipePattern recipe,
        ...
        while (failSafe > 0 && slot.hasStack() && areStacksEqual(slot.getStack(), result)) {
            ...
            // No change in the result slot after shift clicking, let's assume the craft
            // failed and stop here
            else {
                break;
            }
    ...
    private static boolean clearCraftingGridOfItems(RecipePattern recipe,
        ...
        for (int i = 0, slotNum = range.getFirst(); i < slotCount && slotNum < invSlots; i++, slotNum++) {
            ...
            if (slotTmp != null && slotTmp.hasStack() && (clearNonMatchingOnly == false
                    || areStacksEqual(recipe.getRecipeItems()[i], slotTmp.getStack()) == false)) {
                shiftClickSlot(gui, slotNum);
    ...
    public static boolean tryMoveItemsToCraftingGridSlots(RecipePattern recipe,
        ...
        if (range != null && range.getLast() < numSlots && recipe.getRecipeLength() <= range.getSlotCount()) {
            ...
            for (Map.Entry<ItemType, IntArrayList> entry : ingredientSlots.entrySet()) {
                ...
                // Get the actual target slot numbers based on the grid's start and the relative
        ...
        while (true) {
            ...
            if (areStacksEqual(ingredientReference, stackCursor)) {
                ...
                if (isStackEmpty(stackCursor) == false) {
                    ...
                    // Collect all the remaining items into the first found slot, as long as
                    // possible
                    leftClickSlot(gui, slotReturn);

                    // All of them didn't fit into the first slot anymore, switch into the current
                    // source slot
                    if (isStackEmpty(gui.getScreenHandler().getCursorStack()) == false) {
                        slotReturn = slotNum;
                        leftClickSlot(gui, slotReturn);
    ...
    public static void moveAllCraftingResultsToOtherInventory(RecipePattern recipe,
        ...
        if (isStackEmpty(recipe.getResult()) == false) {
            ...
            for (Slot slotTmp : gui.getScreenHandler().slots) {
                // This slot is likely in the player inventory, as there is another inventory
                // above
            ...
            if (slot != null) {
                ...
                IntArrayList slots = getSlotNumbersOfMatchingStacks(gui.getScreenHandler(), slot, true, stackResult,
                        false, false, false);
    ...
    public static void moveOneSetOfItemsFromSlotToPlayerInventory(HandledScreen<? extends ScreenHandler> gui,
        ...
        if (isStackEmpty(stackCursor) == false) {
            IntArrayList slots = getSlotNumbersOfMatchingStacks(gui.getScreenHandler(), slot, false, stackCursor, true,
                    true, false);
    ...
    /**
     * Returns the slot number of the slot that has the smallest stackSize that is
     * still equal to or larger than idealSize. The slot must also NOT be in the
     * same inventory as slotReference. If an adequately large stack is not found,
     ...
     *                               same inventory, if false, then from a different
     *                               inventory
     * @param stackReference
     * @param preferPartial
     * @param treatHotbarAsDifferent
     * @param reverse                if true, returns the slots starting from the
    ...
    private static int getTargetSlot(ScreenHandler container, ItemStack[] originalStacks) {
        ...
        for (int i = 0; i < originalStacks.length; i++) {
            ...
            if ((isStackEmpty(stackOrig) && isStackEmpty(stackNew) == false) || (isStackEmpty(stackOrig) == false
                    && isStackEmpty(stackNew) == false && getStackSize(stackNew) == (getStackSize(stackOrig) + 1))) {
                return i;
            }
        }
    ...
    /*
     * private void clickSlotsToMoveItems(Slot slot, ContainerScreen<? extends
     * Container> gui, boolean matchingOnly, boolean toOtherInventory) { for (Slot
     * slotTmp : gui.getContainer().slots) { if (slotTmp.id != slot.id &&
     * areSlotsInSameInventory(slotTmp, slot) == toOtherInventory &&
     * slotTmp.hasStack() && (matchingOnly == false ||
    ...
    private static void clickSlotsToMoveItemsFromSlot(Slot slotFrom,
        ...
        // Couldn't fit the entire stack to the target inventory, return the rest of the
        // items
        if (isStackEmpty(gui.getScreenHandler().getCursorStack()) == false) {
            leftClickSlot(gui, slotFrom.id);
        }
    ...
    private static void moveMatchingStacksToSlots(HandledScreen<? extends ScreenHandler> gui,
            Slot slot,
            boolean moveUp) {
        IntArrayList matchingSlots = getSlotNumbersOfMatchingStacks(gui.getScreenHandler(), slot, true, slot.getStack(),
                true, true, false);
    ...
    private static Slot moveStackToSlots(HandledScreen<? extends ScreenHandler> gui,
        ...
        for (int slotNum : slotsTo) {
            ...
            if (dstSlot.canInsert(gui.getScreenHandler().getCursorStack()) && (dstSlot.hasStack() == false
                    || areStacksEqual(dstSlot.getStack(), gui.getScreenHandler().getCursorStack()))) {
                leftClickSlot(gui, slotNum);
                lastSlot = dstSlot;
            }
        }
            ...
            if (slotTmp.id != slotIn.id && slotTmp.y != slotIn.y) {
                if (above == slotTmp.y < slotIn.y) {
                    ...
                    if ((isStackEmpty(stackTmp) && slotTmp.canInsert(stackSlot))
                            || (areStacksEqual(stackTmp, stackSlot))
                                    && slotTmp.getMaxItemCount(stackTmp) > getStackSize(stackTmp)) {
    ...
    public static void clickSlot(HandledScreen<? extends ScreenHandler> gui,
        ...
        if (slotNum >= 0 && slotNum < gui.getScreenHandler().slots.size()) {
            ...
            try {
                ...
                ItemScroller.logger.warn(
                        "Exception while emulating a slot click: gui: '{}', slotNum: {}, mouseButton; {}, SlotActionType: {}",
    ...
    /**************************************************************
     * Compatibility code for pre-1.11 vs. 1.11+ Well kind of, as in make the
     * differences minimal, only requires changing these things for the ItemStack
     * related changes.
     *************************************************************/
